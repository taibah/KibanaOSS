...
RUN cd /root && unzip vega_vis-1.1.2--for-Kibana-6.1.4.zip
RUN sed -i 's/"version": "6.1.4"/"version": "6.2.2"/g' /root/kibana/vega_vis/package.json
RUN cd /root && zip -r vega.zip /root/kibana
RUN /bin/bash /usr/share/kibana/bin/kibana-plugin install file:///root/vega.zip || true
